...
> **Correction:** the initial 0.5.30 fallback looked for a duplicate `VERSION` file that release
> directories do not contain. SpawnWP 0.5.31 resolves and validates the active release symlink
> instead. Hosts with the normal installed state file were unaffected.
