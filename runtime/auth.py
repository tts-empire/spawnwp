...
def setup_start(body: SetupStart, request: Request):
    ...
    with db(immediate=True) as connection:
        ...
        if not row or row["used_at"] or row["expires_at"] < now or not hmac.compare_digest(row["code_hash"], _digest(body.bootstrap_code)):
            ...
            raise HTTPException(400, "Setup code is invalid or expired")
    config = _config()
...
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#000;color:#f8f8f8;font:14px system-ui;min-height:100vh;display:grid;place-items:center}.box{width:min(420px,calc(100% - 32px));border-top:2px solid #f6b269;padding:28px 0}h1{font-size:24px}p{color:#a1a1aa;line-height:1.5}label{display:block;margin:14px 0 6px;font-size:11px;text-transform:uppercase;color:#a1a1aa}input,button{width:100%;padding:11px;background:#151518;color:#fff;border:1px solid #3a3a40;border-radius:4px}button{margin-top:14px;background:#f6b269;color:#0d0d10;font-weight:700;cursor:pointer}.alt{background:#202024;color:#fff}.err{color:#ff7373;min-height:20px}.hide{display:none}code{word-break:break-all;color:#f6b269}.codes{white-space:pre-wrap;background:#0d0d10;padding:12px;border:1px solid #2b2b30}#qr{display:block;width:220px;height:220px;margin:16px auto;background:#fff;padding:12px;border-radius:4px}</style></head><body><main class=box><h1>Spawn<span style='color:#f6b269'>WP</span></h1><p id=intro>Secure cockpit access</p>
<section id=login><button onclick=passkey()>Sign in with passkey</button><form onsubmit='fallback(event)'><label>Username</label><input id=user autocomplete=username required><label>Password</label><input id=pass type=password autocomplete=current-password required><label>Authenticator or recovery code</label><input id=second autocomplete=one-time-code required><button class=alt>Use password fallback</button></form></section>
<section id=setup class=hide><form onsubmit='startSetup(event)'><label>One-time setup code</label><input id=boot type=password required><label>Administrator username</label><input id=newuser pattern='[A-Za-z0-9_.-]{3,32}' required><label>Fallback passphrase (14+ characters)</label><input id=newpass type=password minlength=14 required><button>Continue secure setup</button></form><div id=totp class=hide><p>Scan this TOTP code, then register a passkey.</p><img id=qr alt='TOTP QR code'><p>Manual secret: <code id=secret></code></p><label>Current authenticator code</label><input id=otp inputmode=numeric pattern='[0-9]{6}'><button onclick=finishSetup()>Register passkey and finish</button></div><div id=recovery class=hide><p>Store these single-use recovery codes now.</p><div class=codes id=codes></div><button onclick="location.href='/manage'">I saved these codes</button></div></section><p class=err id=err></p></main><script>
let ceremony,creation;const $=id=>document.getElementById(id);function b64(v){let s=btoa(String.fromCharCode(...new Uint8Array(v)));return s.replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'')}function bytes(v){v=v.replace(/-/g,'+').replace(/_/g,'/');return Uint8Array.from(atob(v),c=>c.charCodeAt(0))}function prep(o){o.challenge=bytes(o.challenge);if(o.user)o.user.id=bytes(o.user.id);if(o.excludeCredentials)o.excludeCredentials.forEach(x=>x.id=bytes(x.id));if(o.allowCredentials)o.allowCredentials.forEach(x=>x.id=bytes(x.id));return o}function cred(c){return{id:c.id,rawId:b64(c.rawId),type:c.type,authenticatorAttachment:c.authenticatorAttachment,clientExtensionResults:c.getClientExtensionResults(),response:{clientDataJSON:b64(c.response.clientDataJSON),attestationObject:c.response.attestationObject?b64(c.response.attestationObject):undefined,authenticatorData:c.response.authenticatorData?b64(c.response.authenticatorData):undefined,signature:c.response.signature?b64(c.response.signature):undefined,userHandle:c.response.userHandle?b64(c.response.userHandle):null,transports:c.response.getTransports?c.response.getTransports():[]}}}async function api(url,data){let r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data||{})});let j=await r.json();if(!r.ok)throw Error(j.detail||'Authentication failed');return j}function fail(e){$('err').textContent=e.message||e}async function state(){let s=await fetch('/api/auth/state').then(r=>r.json());if(s.authenticated)location.href='/manage';else if(!s.enrolled){$('login').classList.add('hide');$('setup').classList.remove('hide');$('intro').textContent='Complete one-time administrator setup'}}async function startSetup(e){e.preventDefault();try{let j=await api('/api/auth/setup/start',{bootstrap_code:$('boot').value,username:$('newuser').value,password:$('newpass').value});ceremony=j.ceremony;creation=j.publicKey;$('secret').textContent=j.totp_secret;$('qr').src=j.totp_qr;e.target.classList.add('hide');$('totp').classList.remove('hide')}catch(e){fail(e)}}async function finishSetup(){try{let c=await navigator.credentials.create({publicKey:prep(creation)});let j=await api('/api/auth/setup/finish',{ceremony,credential:cred(c),totp:$('otp').value,passkey_name:'Primary passkey'});$('totp').classList.add('hide');$('recovery').classList.remove('hide');$('codes').textContent=j.recovery_codes.join('\n')}catch(e){fail(e)}}async function passkey(){try{let j=await api('/api/auth/passkey/start',{});let c=await navigator.credentials.get({publicKey:prep(j.publicKey)});await api('/api/auth/passkey/finish',{ceremony:j.ceremony,credential:cred(c)});location.href='/manage'}catch(e){fail(e)}}async function fallback(e){e.preventDefault();try{await api('/api/auth/fallback',{username:$('user').value,password:$('pass').value,second_factor:$('second').value});location.href='/manage'}catch(e){fail(e)}}state().catch(fail);
</script></body></html>"""
